	...
	Meteor.methods({
		...
		emailGonder: function(job) {
			...
			let message = {
				...
				to: job.email,
				subject: `Guaranteed Moving Estimate for ${job.clientFirstName ||
					' '} ${job.clientLastName || ''}`,
				attachment: [
					{
		...
		confirmationGonder: function(job) {
			...
			server.send(message, function(err) {
				err
					? console.error(`id: ${job._id}`, err)
					: console.info('Email succesfully sent to: ' + job.email);
			});
		},
...
`,
			...
			server.send(message, function(err, message) {
				if (err) {
					console.error(
						`Error while trying send email to cardholder. id: ${obj._id}` + err
					);
					throw new Meteor.Error(
