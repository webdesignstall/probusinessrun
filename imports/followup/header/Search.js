                ...
                if (err) {
                    ...
                    arrayOfWords.map((word) => {
                        this.state.works.map((work) => {
                            work.clientFirstName && (work.clientFirstName.toLowerCase().search(word.toLowerCase())) > -1 ? result.add(work) : null;
                            work.clientLastName && (work.clientLastName.toLowerCase().search(word.toLowerCase())) > -1 ? result.add(work) : null;
                            work.jobNumber && (work.jobNumber.toLowerCase().search(word.toLowerCase())) > -1 ? result.add(work) : null;
                            work.phoneNumber && ((work.phoneNumber).toString().toLowerCase().search(word.toLowerCase())) > -1 ? result.add(work) : null;
                        });
                    });
