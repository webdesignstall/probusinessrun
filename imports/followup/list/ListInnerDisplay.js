...
export default class ListInnerDisplay extends TrackerReact(Component) {
	...
	componentDidMount() {
		this.x = Tracker.autorun(() => {
			this.displayExtendedJobInformation = this.context.displayExtendedJobInformation;
			this.setDisplayExtendedInformation = this.context.setDisplayExtendedInformation;
	...
	showMore() {
		...
		if (this.state.job._id === session && session !== '') {
			...
			Session.set('job_', {});
			this.setState({ show: '' });
			this.setDisplayExtendedInformation(false);
		} else {
			Session.set('is', this.state.job._id);
			Session.set('ExtendedJobInformation', this.state.job._id);
			// this.setDisplayExtendedInformation(this.state.job_id);
			let job = this.workData(this.state.job._id);
			Session.set('job_', this.state.job);
			this.setState({
				show: this.state.job._id
			});
			this.setDisplayExtendedInformation(this.state.job);
		}
	}
			...
			<div className={this.state.job._id ? '' : 'hide'} style={{ position: 'relative' }}>
				...
				<span>
					<CustomerPriority
						...
						rate={this.state.job.customerRate || 0}
					/>
				</span>
	...
	render() {
		return (
			<React.Fragment>
				{this.state.job && this.state.job._id ? this.displayInfo() : 'NO RESULT'}
			</React.Fragment>
		);
	}
}
