...
export default class ArrivalWindow extends React.Component {
    ...
    renderOptions() {
        return (
            this.state.options.map((option) => {
                return (
                    <option key={option.name} value={option.name} disabled={option.status === 0 ? true : false}>{option.name}</option>
                );
            })
        );
    }
    ...
    renderArrivalTime() {
        return (
            <select name="select-arrive-time" id="select-arrive-time" defaultValue={this.state.defValue} >
                {this.renderOptions()}
            </select>
        );
    }
    ...
    custom() {
        return (
            <input id="input-custom-time" className={this.state.custom ? '' : 'hide'} type="text" value={this.state.valueOfInput} onChange={this.onChangeInput} placeholder="Input the arrival time window" />
        );
    }

    render() {
        return (
            <div className={this.state.custom ? 'deep-purple lighten-5' : ''} >
