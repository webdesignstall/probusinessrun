...
class CardHolderMessage extends Component {
        ...
        this.state = {
            ...
            messages: {
                ...
                hmlt: (rate, laborTime, jobNumber) =>
                    `Amount to Charge:  $${rate} per hour with ${laborTime} hours minimum charge plus if any additional charge for listed moving services with the job number: ${jobNumber}`,
    ...
    renderMessage(job) {
        ...
        let flatRate = (job.flatRate && job.flatRate[0] && job.flatRate[0].isTrue && job.flatRate[0].cardAmount) || null;
    ...
    render() {
        return (
            <div className="col s12 m6 l6 offset-m3 offset-l3">
                ...
                <p>
                    I authorize {this.state.job && this.state.job.companyInfo.name} to charge the amount listed above to the
                    credit card provided herein on my behalf. I agree to pay for this purchase in accordance with the issuing bank
                    cardholder agreement. I understand this move is scheduled for{' '}
                    {this.state.job && this.state.job.clientFirstName} {this.state.job && this.state.job.clientLastName} and I
                    agree to pay behalf of {this.state.job && this.state.job.clientFirstName}{' '}
